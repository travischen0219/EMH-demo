...
parser = argparse.ArgumentParser(
    description="Code to generate results on ReSyncED evaluation set"
)

parser.add_argument("--mode", type=str, help="random | dubbed | tts", required=True)

parser.add_argument(
    "--filelist", type=str, help="Filepath of filelist file to read", default=None
)

parser.add_argument(
    "--results_dir", type=str, help="Folder to save all results into", required=True
)
...
parser.add_argument(
    "--face_det_batch_size",
    type=int,
    help="Single GPU batch size for face detection",
    default=16,
)

parser.add_argument(
    "--wav2lip_batch_size", type=int, help="Batch size for Wav2Lip", default=128
)
parser.add_argument(
    "--face_res",
    help="Approximate resolution of the face at which to test",
    default=180,
)
parser.add_argument(
    "--min_frame_res",
    help="Do not downsample further below this frame resolution",
    default=480,
)
parser.add_argument(
    "--max_frame_res", help="Downsample to at least this frame resolution", default=720
)
...
def rescale_frames(images):
    ...
    for factor in range(2, 16):
        ...
        if min(h // factor, w // factor) < args.min_frame_res:
            break
        if np.abs(downsampled_res - args.face_res) >= diff:
            break
    ...
    return [
        cv2.resize(im, (im.shape[1] // (factor), im.shape[0] // (factor)))
        for im in images
    ]
...
def face_detect(images):
    ...
    results = [
        [image[y1:y2, x1:x2], (y1, y2, x1, x2), True]
...
def datagen(frames, face_det_results, mels):
    ...
    for i, m in enumerate(mels):
        if i >= len(frames):
            raise ValueError("Equal or less lengths only")
